...
describe('SurrealDB Simple CRUD', () => {
  ...
  test('should update a record', async () => {
    ...
    const updated = await db.query(`UPDATE ${person.id} MERGE $data`, { data: { name: 'Jane Smith' } }).collect() as any;
    
    const up = updated as any;
    const item = Array.isArray(up) && Array.isArray(up[0]) ? up[0][0] : (Array.isArray(up) ? up[0] : up);
