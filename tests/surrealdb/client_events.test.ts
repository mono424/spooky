...
describe("Client Spooky Events", () => {
  ...
  test("CREATE sets IsDirty=true and calculates IntrinsicHash", async () => {
    ...
    const result: any = await (db.query("SELECT * FROM _spooky_data_hash WHERE RecordId = $id", { id: user.id }) as any).collect();
    ...
    const hashRecord = (queryResult && queryResult.result && Array.isArray(queryResult.result)) 
  ...
  test("UPDATE sets IsDirty=true and updates IntrinsicHash", async () => {
    ...
    // Get initial hash
    const res1: any = await (db.query("SELECT * FROM _spooky_data_hash WHERE RecordId = $id", { id: user.id }) as any).collect();
    const qr1 = res1[0];
    const hash1 = (qr1 && qr1.result && Array.isArray(qr1.result)) ? qr1.result[0] : (Array.isArray(qr1) ? qr1[0] : qr1);

    // Reset dirty flag
    await db.query("UPDATE _spooky_data_hash SET IsDirty = false WHERE RecordId = $id", { id: user.id });
    ...
    const res2: any = await (db.query("SELECT * FROM _spooky_data_hash WHERE RecordId = $id", { id: user.id }) as any).collect();
    const qr2 = res2[0];
    const hash2 = (qr2 && qr2.result && Array.isArray(qr2.result)) ? qr2.result[0] : (Array.isArray(qr2) ? qr2[0] : qr2);
  ...
  test("DELETE sets PendingDelete=true", async () => {
    ...
    const qr = res2[0];
    const hashRecord = (qr && qr.result && Array.isArray(qr.result)) ? qr.result[0] : (Array.isArray(qr) ? qr[0] : qr);
