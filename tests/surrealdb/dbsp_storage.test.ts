...
describe('DBSP Storage Persistence', () => {
  ...
  test('Verify MutationManager.create transaction logic (Validation)', async () => {
    // This replicates the SQL logic from MutationManager.create
    // to ensure the transaction structure, field names, and return values are correct.

    const threadId = 'thread:mutation_test_' + Date.now();
    const mutationId = '_spooky_pending_mutations:' + Date.now();
    const data = {
      title: 'Mutation Test Thread',
      content: 'Content',
      author: 'user:someauthor', // Mock author for schema validation
    };

    const query = `
          BEGIN TRANSACTION;
          
          LET $created = CREATE ONLY ${threadId} CONTENT $data;
          LET $mutation = CREATE ONLY ${mutationId} CONTENT {
              mutationType: 'create',
              recordId: $created.id,
              data: $data
          };
    ...
    try {
      ...
      // SurrealDB response format: [ { result: { target: ..., mutation_id: ... } } ] depending on client
      // With `.collect()` helper used in this file? No, direct .query used here?
      // Wait, other tests use runQuery helper.
      // Let's use runQuery logic but customized.

      const result = (res as any)[0].result || (res as any)[0];

      expect(result).toBeDefined();
      // Since we return an object, result is that object
      expect(result.target).toBeDefined();
      expect(result.mutation_id).toBeDefined();
      expect(result.target.id).toBe(threadId);

      // Verify Mutation Record exists
      // Verify Mutation Record exists
      const rid = new RecordId('_spooky_pending_mutations', mutationId.split(':')[1]);
      const mutRec = ((await db.select(rid)) as any)[0] || (await db.select(rid));
      expect(mutRec).toBeDefined();
      // With recent core/syncgen changes, these should be camelCase
      expect((mutRec as any).mutationType).toBe('create');
      expect((mutRec as any).data.title).toBe(data.title);
    } catch (e) {
      console.error('Mutation Test Failed:', e);
      throw e; // Fail test
    }
  });
});
