...
describe('Blake3 XOR Logic', () => {
  ...
  test('should XOR two hashes correctly', async () => {
    ...
    // Because A XOR A = 0
    const resultSame = (await db
      .query(`RETURN mod::xor::blake3_xor('${hashA}', '${hashA}');`)
      .collect()) as any;
    ...
    const resultNeutral = (await db
      .query(`RETURN mod::xor::blake3_xor('${hashA}', '${zeroHash}');`)
      .collect()) as any;
    ...
    const resAB = (await db
      .query(`RETURN mod::xor::blake3_xor('${hashA}', '${hashB}');`)
      .collect()) as any;
    const resBA = (await db
      .query(`RETURN mod::xor::blake3_xor('${hashB}', '${hashA}');`)
      .collect()) as any;
