...
async function getContainer() {
  ...
  const modulesDir = path.resolve(__dirname, '../../tests/.sp00ky');
...
export async function createTestDb() {
  ...
  // Load generated schema from .sp00ky
  const fs = require('fs');
  const path = require('path');
  const schemaPath = path.resolve(__dirname, '../../tests/.sp00ky/schema.gen.surql');

  if (fs.existsSync(schemaPath)) {
    console.log('Loading generated schema from:', schemaPath);
    let schema = fs.readFileSync(schemaPath, 'utf8');

    // No more mocking! Using real WASM module from .sp00ky
