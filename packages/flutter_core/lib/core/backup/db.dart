...
class DatabaseService {
  ...
  Future<void> init() async {
    ...
    local = await connectDb(path: config.database.path);
    ...
    if (config.database.endpoint != null && config.database.endpoint!.isNotEmpty) {
  ...
  Future<T> runWithInternal<T>(Future<T> Function(SurrealDatabase) fn) async {
    ...
    try {
      await local.useDb(db: config.database.internalDatabase ?? config.database.database);
