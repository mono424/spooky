...
pub fn load_config<P: AsRef<Path>>(spooky_config_path: P) -> Result<JobConfig> {
    ...
    let spooky_config: SpookyConfig = serde_yaml::from_str(&config_str)
        .context("Failed to parse spooky config")?;
    ...
    for (backend_name, backend_config) in spooky_config.backends {
        ...
        if backend_config.method.method_type == "outbox" {
            if let (Some(base_url), Some(table)) = (backend_config.base_url, backend_config.method.table) {
