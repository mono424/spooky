...
export class UpQueue {
    ...
    async loadFromDatabase() {
        try {
            const rawResult = await this.local.query<any>(`SELECT * FROM _spooky_pending_mutations ORDER BY created_at ASC`);
            ...
            this.queue = records.map((r: any) => {
                switch (r.mutation_type) {
                    case "create":
                        return { type: "create", mutation_id: r.id, data: r.data };
                    case "update":
                        return { type: "update", mutation_id: r.id, record_id: r.record_id, data: r.data };
                    case "delete":
                        return { type: "delete", mutation_id: r.id, record_id: r.record_id };
                    default:
                        console.warn(`Unknown mutation type: ${r.mutation_type}`, r);
                        return null;
                }
