...
async fn test_storage_mode_memory() {
    ...
    db.query("CREATE foo SET val = 1".into(), "".into()).await.unwrap();
    let res = db.query("SELECT * FROM foo".into(), "".into()).await.unwrap();
    println!("MEM RES: {}", res);
...
async fn test_storage_mode_disk() {
    ...
    db.use_db("test".into(), "test".into()).await.unwrap();
    db.query("CREATE bar SET val = 2".into(), "".into()).await.unwrap();
    ...
    let res = db2.query("SELECT * FROM bar".into(), "".into()).await.unwrap();
