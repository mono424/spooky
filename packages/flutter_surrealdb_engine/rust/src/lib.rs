            ...
            match response_result {
                Ok(mut response) => {
                     ...
                     for i in 0..num_statements {
                         let result: Result<surrealdb_types::Value, _> = response.take(i);
                         match result {
                             Ok(val) => {
                                 let raw_json = serde_json::to_value(&val).unwrap_or(serde_json::Value::Null);
                                 //let json_val = fix_surreal_json(raw_json);
                                 let json_str = serde_json::to_string(&raw_json).ok();
...
fn fix_surreal_json(val: serde_json::Value) -> serde_json::Value {
    match val {
        serde_json::Value::Object(map) => {
            // Check if it is a single-key object which serves as a type wrapper.
            // Common wrappers in surrealdb serialization: Strand, Datetime, Thing, Array, Object, etc.
            if map.len() == 1 {
                let (key, value) = map.iter().next().unwrap();
                match key.as_str() {
                    "Strand" | "Datetime" | "Thing" | "Uuid" | "Duration" => {
                        return fix_surreal_json(value.clone());
                    },
                    "Array" | "Object" => {
                        return fix_surreal_json(value.clone());
                    },
                    ...
                    _ => {} // Not a wrapper, proceed to process recursively
                }
            }
            
            let mut new_map = serde_json::Map::new();
            for (k, v) in map {
                 new_map.insert(k, fix_surreal_json(v));
            }
            serde_json::Value::Object(new_map)
        },
        serde_json::Value::Array(arr) => {
             serde_json::Value::Array(arr.into_iter().map(fix_surreal_json).collect())
        },
        _ => val
    }
}
