...
fn main() {
    ...
    // Now simulate Sp00kyValue conversion (if possible, but since we are in ssp-protocol we can't depend on ssp).
    // Let's manually do the equivalent of Sp00kyValue int conversion:
    // When Value -> Sp00kyValue -> Value happens, ints become json!(i64).
