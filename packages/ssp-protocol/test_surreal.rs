...
fn main() {
    ...
    // parse as surrealdb value
    let surreal_val: surrealdb::sql::Value = surrealdb::sql::json(json_str).unwrap();
    let sdk_val = surrealdb::types::Value::from(surreal_val);
    let json_val = sdk_val.into_json_value();

    let rows: Vec<Value> = match json_val {
        Value::Array(a) => a,
        _ => panic!("Not array"),
    };
    ...
    println!("HASH VIA SURREALDB: {}", hash);
}
