...
fn benchmark_latency_mixed_stream() {
    ...
    writeln!(writer, "views,records,total_time_ms,latency_last_50_ms,ops_per_sec").unwrap();

    println!("Start Benchmark ({} Records Total, Batch Size {})...", TOTAL_RECORDS, BATCH_SIZE);
    ...
    for &view_count in &VIEW_COUNTS {
        ...
        let last_valid_thread_id = prepared_stream.iter().rev()
            ...
            .map(|r| r.record.get("thread").unwrap().as_str().unwrap().to_string())
        ...
        for chunk in prepared_stream.chunks(BATCH_SIZE) {
            ...
            writeln!(writer, "{},{},{:.2},{:.4},{:.2}", 
                view_count, global_record_count, total_ms, latency_last_50_ms, ops_sec).unwrap();
            
            // Terminal Anzeige (Live Update)
            print!("\r>> Views: {} | Records: {}/{} | Latency: {:.3} ms | Speed: {:.0} ops/sec", 
        ...
        if !view.cache.contains_key(last_valid_thread_id.as_str()) {
            panic!("\nFEHLER: Circuit Update unvollständig! Thread {} fehlt im View.", last_valid_thread_id);
        }
...
fn create_magic_comments_plan(view_id: &str) -> QueryPlan {
    let scan_threads = Operator::Scan { table: "thread".to_string() };
    let scan_authors = Operator::Scan { table: "author".to_string() };
    ...
    let scan_comments = Operator::Scan { table: "comment".to_string() };
