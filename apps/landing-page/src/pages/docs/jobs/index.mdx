...
DEFINE FIELD delay ON TABLE job TYPE int DEFAULT ALWAYS 0
  PERMISSIONS FOR create, select WHERE true FOR update WHERE false;
-- Seconds, not milliseconds. It also sizes the reclaim lease (below).
...
DEFINE FIELD assignee ON TABLE job TYPE option<string>
  PERMISSIONS FOR select WHERE true FOR create, update WHERE false;
-- The lease: until when the current attempt may hold this row, and the fencing
-- token it must present to write the outcome.
DEFINE FIELD lease_until ON TABLE job TYPE option<datetime>
  PERMISSIONS FOR select WHERE true FOR create, update WHERE false;
DEFINE FIELD lease_epoch ON TABLE job TYPE option<int>
  PERMISSIONS FOR select WHERE true FOR create, update WHERE false;
...
retention:
  ...
  runDeadline: 1h    # ceiling on ONE run, for schedules with no deadline: of their own`;
...
4. **A recovery sweep catches what events miss:** rows created while the SSP was down, jobs whose delay elapsed during a restart, and `processing` rows whose lease has run out.
5. **Every attempt holds a lease, and it expires.** Claiming a job stamps `lease_until` (the job's own `timeout` plus a grace margin) and bumps `lease_epoch`. A `processing` row past its lease is handed back to the queue **regardless of whether the SSP that claimed it is still alive** — "that process is up" and "this work is progressing" are different questions, and an SSP that restarts under the same id, or one that is heartbeating but has lost its queue, answers the first one yes while holding rows it will never finish. A dead owner only makes the reclaim happen sooner.

   The lease covers one *attempt*, not the whole retry budget: a job that fails and backs off is claimed again and gets a fresh window, so nothing has to renew anything.

   `lease_epoch` is what makes reclaiming safe. Each of the runner's writes is guarded on the token it claimed under, so if a slow attempt wakes up after its row was reclaimed and re-run, its write matches nothing instead of overwriting the attempt that replaced it. Without that, reclaiming a stuck job would trade it for a corrupted one.

<Warning>
  A SCHEMAFULL job table must define the platform's own fields, or those writes are rejected at the statement level and quietly lost. `spky deploy` injects `assignee`, `lease_until`, `lease_epoch`, `result`, `errors[*]`, and `timeout` with `IF NOT EXISTS` (plus the retention index below), so re-deploying an older project fixes it.

  Until it does, that table's jobs are claimed without a lease and fall back to the older rule: a `processing` row is reclaimed once it has been untouched for ten minutes. Nothing breaks, and nothing gains a per-job deadline either.
...
| `runDeadline` | 1h | default wall-clock ceiling on one schedule/workflow run; `0` disables it. See [Run deadlines](/docs/jobs/schedules#run-deadlines). |
...
<Note>
  `status`, `errors`, `result`, `assignee`, `lease_until`, and `lease_epoch` are platform-written: clients may read them but never set them. `assignee` is the owning SSP node id; the two `lease_*` fields are how a stuck job is reclaimed, so a client that could forge them could keep a row unreclaimable forever.
</Note>
