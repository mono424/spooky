...
<Warning>
  A step's output is its HTTP response body, captured on the job row. Keep it small (identifiers, counts, paths) rather than the data itself. Bodies over 64 KB are replaced with a marker and the step still succeeds, so a large response fails quietly as *missing input* to the next step rather than as an error.
</Warning>
