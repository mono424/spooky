...
These four fields on `_00_workflow_run` exist for this: `trigger` (`cron` or `manual`, unset on rows older than the field), `rerun_of`, `retry_count` (default `0`) and `last_retry_at`. They ship with `spky deploy`; a scheduler upgraded ahead of the deploy answers rerun and retry with a `409` that says so.
