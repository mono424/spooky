...
(`SPKY_DB_WS`, `SPKY_DB_NS`, `SPKY_DB_NAME`, `SPKY_DB_USER`,
`SPKY_DB_PASS`, `SPKY_SCHEDULER_ID`,
`SPKY_SNAPSHOT_UPDATE_INTERVAL_SECS`, `SPKY_SCHEDULER_BACKENDS`).
...
- `SPKY_DB_WS` - SurrealDB WebSocket URL (overrides `db.url`)
...
- `SPKY_SCHEDULER_BACKENDS` - JSON-encoded backend list (job-runner configuration)
