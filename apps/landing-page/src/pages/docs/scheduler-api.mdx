...
- `SP00KY_SCHEDULER_INGEST_HOST` - Host to bind to (default: `0.0.0.0`)
- `SP00KY_SCHEDULER_INGEST_PORT` - Port to bind to (default: `9667`)
...
- `SP00KY_SCHEDULER_DB_URL` - Database URL (default: `localhost:8000/rpc`)
- `SP00KY_SCHEDULER_DB_NAMESPACE` - Database namespace
- `SP00KY_SCHEDULER_DB_DATABASE` - Database name
- `SP00KY_SCHEDULER_LOAD_BALANCE` - Load balance strategy
- `SP00KY_SCHEDULER_HEARTBEAT_INTERVAL_MS` - Heartbeat interval
- `SP00KY_SCHEDULER_HEARTBEAT_TIMEOUT_MS` - Heartbeat timeout
- `SP00KY_SCHEDULER_BOOTSTRAP_CHUNK_SIZE` - Bootstrap chunk size
- `SP00KY_SCHEDULER_BOOTSTRAP_TIMEOUT_SECS` - Bootstrap timeout (default: `120`)
- `SP00KY_SCHEDULER_INGEST_HOST` - HTTP server host
- `SP00KY_SCHEDULER_INGEST_PORT` - HTTP server port
- `SP00KY_SCHEDULER_REPLICA_DB_PATH` - RocksDB snapshot replica path (default: `./data/replica`)
- `SP00KY_SCHEDULER_WAL_PATH` - Write-ahead log path (default: `./data/event_wal.log`)
- `SP00KY_SCHEDULER_SSP_POLL_INTERVAL_MS` - SSP health poll interval during bootstrap (default: `3000`)
- `SP00KY_SCHEDULER_MAX_BUFFER_PER_SSP` - Max buffered messages per SSP (default: `10000`)
- `SP00KY_SCHEDULER_SNAPSHOT_UPDATE_INTERVAL_SECS` - Snapshot update interval (default: `300`)
