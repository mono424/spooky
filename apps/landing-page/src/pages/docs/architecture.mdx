...
The Sidecar listens to changes in the database and computes the Merkle Tree hashes required for the sync engine to work. It writes these hashes back to the `_00_*` system tables.
...
Configure via `load_balance` in `sp00ky.yml` or `SP00KY_SCHEDULER_LOAD_BALANCE` environment variable.
