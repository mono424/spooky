...
description: "Encrypted environment variables: set them once, share them with your team, inject them at deploy time."
---
...
export const code30 = `# 1. Initialize your encryption vault
spky env unlock
...
export const code29 = `spky env unlock`;

export const code28 = `# With a name, goes straight to the value prompt
spky env set DATABASE_URL

# Without a name, prompts for the name first
spky env set`;

export const code27 = `# Load development variables (default) and start your app
spky env pull && pnpm dev

# Load production variables
spky env pull --prod && pnpm start`;

export const code26 = `# Export into the current shell
eval "$(spky env pull)"

# Or use with env
env $(spky env pull | xargs) pnpm dev`;

export const code25 = `spky env list`;

export const code24 = `NAME                           DEV    PROD   UPDATED
-----------------------------------------------------------------
DATABASE_URL                   yes    yes    2026-04-08
STRIPE_SECRET_KEY              yes    yes    2026-04-08
DEBUG_MODE                     yes    -      2026-04-08`;

export const code23 = `spky env rm DATABASE_URL`;

export const code22 = `spky env import .env`;

export const code21 = `spky env passphrase`;

export const code20 = `# Request a reset (admins are notified by email)
spky env reset request

# After admin approves, set your new passphrase
spky env reset complete`;

export const code19 = `rm ~/.sp00ky/vault-derived-key`;

export const code18 = `spky env <COMMAND>
...
export const code17 = `POST /v1/vault/init
...
export const code16 = `GET /v1/vault/status`;

export const code15 = `{ "initialized": true }`;

export const code14 = `POST /v1/vault/change-passphrase
Content-Type: application/json
...
export const code13 = `POST /v1/tenants/{tenantID}/vault-resets`;

export const code12 = `GET /v1/tenants/{tenantID}/vault-resets`;
...
export const code10 = `GET /v1/tenants/{tenantID}/vault-resets/mine`;

export const code9 = `POST /v1/tenants/{tenantID}/vault-resets/{resetID}/approve
Content-Type: application/json
...
export const code8 = `POST /v1/tenants/{tenantID}/vault-resets/complete
Content-Type: application/json
...
export const code7 = `DELETE /v1/tenants/{tenantID}/vault-resets/{resetID}`;

export const code6 = `PUT /v1/projects/{projectID}/envs/{NAME}
Content-Type: application/json
...
export const code5 = `GET /v1/projects/{projectID}/envs`;
...
export const code3 = `POST /v1/projects/{projectID}/envs/load
Content-Type: application/json
...
export const code1 = `DELETE /v1/projects/{projectID}/envs/{NAME}`;
...
All values are encrypted at rest using the tenant's [Shared Vault](/docs/reference/vault). For details on the encryption model, key hierarchy, and security properties, see the vault documentation.
...
1. **Choose a vault passphrase**: this protects your encryption keys, so pick something strong
...
<Note title="Teams">
The first team member to run <code>env unlock</code> creates the tenant vault. Other members receive vault access through the invitation flow. They set their own passphrase when accepting the invitation. See <a href="/docs/reference/vault">Shared Vault</a> for details.
</Note>
...
In non-interactive environments (CI/CD), set the <code>SPOOKY_API_KEY</code> environment variable for authentication and cache the derived key in <code>~/.sp00ky/vault-derived-key</code> during your pipeline setup.
</Note>
...
See [Passphrase Reset](/docs/reference/vault#passphrase-reset) for the full flow.
