---
layout: ../../../layouts/DocsLayout.astro
title: Teams
description: Invite members, manage roles, and handle vault passphrase resets.
---

import CodeBlock from '../../../components/ui/CodeBlock.astro';
import Warning from '../../../components/ui/Warning.astro';
import Note from '../../../components/ui/Note.astro';

export const code9 = `spky team invite alice@example.com`;

export const code8 = `spky team invites`;

export const code7 = `spky team revoke alice@example.com`;

export const code6 = `spky team list`;

export const code5 = `EMAIL                          ROLE       JOINED
------------------------------------------------------------
khadim@example.com             admin      2026-03-01
alice@example.com              member     2026-03-15
bob@example.com                member     2026-04-02`;

export const code4 = `spky team remove alice@example.com`;

export const code3 = `# Member requests a reset (admins are notified by email)
spky env reset request

# Admin approves the reset
spky env reset approve alice@example.com

# Member completes the reset with a new passphrase
spky env reset complete`;

export const code2 = `spky team rename "My Team"`;

export const code1 = `spky team <COMMAND>

Commands:
  list               List all team members
  invite <EMAIL>     Invite a user by their GitHub email
  invitations        List pending invitations
  revoke <EMAIL>     Revoke a pending invitation
  remove <EMAIL>     Remove a team member
  rename [NAME]      Rename the tenant`;

Sp00ky Cloud projects live inside a **tenant**. Every tenant has at least one admin and can have additional team members who share access to projects, deployments, and the [Shared Vault](/docs/reference/vault).

## Plan Limits

| Plan | Included Members | Additional Members |
| :--- | :--- | :--- |
| **Starter** | 1 (admin only) | +$5/mo each |
| **Pro** | Up to 5 | +$5/mo each |

Additional team members beyond the plan limit are billed as a metered add-on at $5 per member per month.

## Inviting Team Members

Invite a new member by their GitHub email address:

<CodeBlock
  code={code9}
  lang="bash"
/>

If the [Shared Vault](/docs/reference/vault) is initialized, the CLI will prompt for your vault passphrase to securely transfer vault access to the new member. The invited user receives an email with a link to accept. They'll authenticate via GitHub OAuth and set their own vault passphrase.

<Note title="GitHub Account Required">
Team members must have a GitHub account with the invited email address. Sp00ky Cloud uses GitHub OAuth for authentication.
</Note>

### Pending Invitations

View all pending invitations:

<CodeBlock
  code={code8}
  lang="bash"
/>

Revoke a pending invitation:

<CodeBlock
  code={code7}
  lang="bash"
/>

Invitations expire after 7 days.

## Listing Team Members

<CodeBlock
  code={code6}
  lang="bash"
/>

Example output:

<CodeBlock
  code={code5}
  lang="bash"
/>

## Removing Team Members

<CodeBlock
  code={code4}
  lang="bash"
/>

<Warning>
Removing a team member immediately revokes all access. Their copy of the vault encryption key is deleted and any API keys they created are revoked. This cannot be undone. You'll need to re-invite them to restore access.
</Warning>

If a removed member previously loaded sensitive values, consider rotating those variables with `spky env set`.

## Roles

| Role | Capabilities |
| :--- | :--- |
| **Admin** | Full access. Manage billing, invite/remove members, create/destroy projects, deploy, approve vault resets. |
| **Member** | Access all projects. Deploy, view logs, manage environment variables via shared vault. Cannot manage billing or remove other members. |

The user who creates the first project becomes the tenant's admin. Additional admins can be added by inviting with the admin role.

## Vault Passphrase Reset

If a team member loses their vault passphrase, an admin can approve a reset so they can set a new one. This does not affect other members' passphrases or any encrypted data.

<CodeBlock
  code={code3}
  lang="bash"
/>

See [Passphrase Reset](/docs/reference/vault#passphrase-reset) for the full details.

## Renaming a Tenant

<CodeBlock
  code={code2}
  lang="bash"
/>

## CLI Reference

<CodeBlock
  code={code1}
  lang="bash"
/>
