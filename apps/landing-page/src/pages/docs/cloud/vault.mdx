...
```
Member passphrase
  └─ Argon2id ──▶ Derived key (32 bytes, per-member)
                    └─ decrypts ──▶ Encryption key (shared secret, one copy per member)
                                      └─ decrypts ──▶ Tenant RSA-4096 private key (one per tenant)

Per variable:
  Random AES-256 key
    ├─ encrypts ──▶ Variable value (AES-256-GCM)
    └─ encrypted by ──▶ Tenant RSA public key (RSA-4096 OAEP)
```
