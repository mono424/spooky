...
description: How environment variables are encrypted, what the key hierarchy protects, and what it does not.
---
...
export const code4 = `spky env passphrase`;

export const code3 = `rm ~/.sp00ky/vault-derived-key`;

export const code2 = `# Step 1: Request a reset
spky env reset request

# Step 2: Admin approves (enters their passphrase)
spky env reset approve alice@example.com

# Step 3: Set your new passphrase
spky env reset complete`;

export const code1 = `# List pending requests
spky env reset list`;
...
The vault is initialized the first time any team member runs `spky env unlock`. Subsequent members receive access through the [invitation flow](/docs/cloud/team#inviting-team-members).
...
Variable decryption happens on the server, not on the client. However, your passphrase never leaves your machine; only the derived key is sent to the server over TLS. The derived key is used only in memory and is never persisted server-side. The encryption protects data <strong>at rest</strong> in the database, not in transit between the server and CLI (that's handled by TLS).
</Note>
...
- **Compromised server.** Because variable decryption happens server-side, a compromised server process could observe plaintext values during `env pull` or `env set`. The encryption is at-rest protection, not end-to-end. However, the passphrase itself never reaches the server, only the derived key.
...
When a member is removed (`spky team remove`):
...
The removed member can no longer decrypt any variables. Their path to the tenant's private key is destroyed. If you suspect a removed member had access to sensitive values, rotate those values with `spky env set`.
...
- **Rotate variables when compromised.** If you suspect a value was leaked, run `spky env set` with a new value rather than rotating passphrases.
- **Limit team size.** Only invite members who need access to production secrets.
- **Audit regularly.** Use `spky team list` to review who has access. Remove members who no longer need it.
