...
The scheduler listens on **two** ports, and the distinction matters for how you
expose it:
...
startup and after every snapshot drain that leaves nothing buffered, and by
default re-clones the replica and re-bootstraps every SSP when a table is
empty in the replica but not upstream, or when any other count mismatch
persists across consecutive checks.
...
`drift.stuck` lists tables an automatic re-clone did not fix; those need an
operator (a row the replica schema rejects, typically). `drift.upstream` is
...
- `SPKY_DRIFT_AUTO_RECLONE` - Re-clone the replica and re-bootstrap every SSP when the check finds the replica missing rows (default: `true`; `false` reports only, via `/health/snapshot`, `/metrics` and `spky verify`)
