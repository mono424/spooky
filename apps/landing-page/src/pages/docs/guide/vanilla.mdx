...
<CodeBlock code={`npm install @spooky-sync/core surrealdb`} lang="bash" />
...
<CodeBlock 
  code={`spky --input src/schema.surql --output src/schema.gen.ts --format typescript`}
  lang="bash"
/>
...
<CodeBlock
  fileName="src/db.ts"
  lang="typescript"
  code={`import { Sp00kyClient, type Sp00kyConfig } from '@spooky-sync/core';
