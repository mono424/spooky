...
Every step is an ordinary atomic job, so the whole [jobs](/docs/backend/jobs) lifecycle applies per step: retries, timeouts, `spky jobs` visibility, kill, and crash recovery. A workflow adds the ordering, the data passing, and the run-level view.
...
<Note>
  A workflow with a `schedule:` runs on a clock exactly like a [schedule](/docs/backend/schedules) does — same cron/interval syntax, same `spky schedules pause` / `trigger`. Omit `schedule:` for a workflow you only ever trigger by hand.
</Note>
...
`concurrency` is then per row as well, so a tenant whose report is still generating doesn't block anyone else's. The [Schedules](/docs/backend/schedules#concurrency) page covers the three policies.
...
In `watch`, the selected step's payload, output, and errors appear underneath the graph — `↑↓`/`jk` to move, `K` to kill the run, `r` to refresh now, `q` to quit.
...
- [Schedules](/docs/backend/schedules) — single-route cron/interval jobs, fan-out, and the `spky schedules` commands
- [Jobs](/docs/backend/jobs) — the job lifecycle every step inherits
- [Configuration](/docs/configuration#schedules-and-workflows) — where `workflows:` sits in `sp00ky.yml`
