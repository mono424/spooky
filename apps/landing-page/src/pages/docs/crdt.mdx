...
<Experimental>
  CRDT Fields are experimental. The API may change between canary releases. Only `text` has a built-in editor today.
</Experimental>
...
For a full-featured version with cursor presence, single-line mode, and placeholder support, see [`CollaborativeEditor.tsx`](https://github.com/piesocket/sp00ky/blob/main/example/app-solid/src/components/CollaborativeEditor.tsx) in the example app.
...
- **Text only** for now. `map`/`list`/`counter` need manual `LoroDoc` wiring.
