...
When `useCrdtField` mounts, the `CrdtManager` ensures a single `LIVE SELECT * FROM <table>` is open for that table (shared across every open field on that table), imports the existing snapshot, and pushes local changes back debounced at `crdtDebounceMs` (default 500 ms, configurable on `Sp00kyConfig`). Cross-user propagation also rides the list_ref sync path so updates arrive even when SurrealDB's cross-session LIVE-permission gap drops the parent-table notification (see [Architecture](/docs/architecture)).
