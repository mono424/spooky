...
Each backend in `spooky.yml` can declare an optional `dev` field. When you run `spooky dev`, these commands are spawned alongside the rest of the infrastructure (SurrealDB, SSP, scheduler), so you don't need to start backend services in separate terminals.
...
<CodeBlock
  ...
  api:
    dev:
      ...
      workdir: ../api       # optional: directory to run from (relative to spooky.yml)
    # ...other fields...`}
  lang="yaml"
/>
...
<CodeBlock
  ...
  ml-service:
    dev:
      ...
      workdir: ../ml-service  # optional: directory to run from (relative to spooky.yml)
    # ...other fields...`}
  lang="yaml"
/>
...
All typed forms support an optional `workdir` field that is resolved relative to the project root (the directory containing `spooky.yml`). If omitted, commands run from the project root.
