...
- **`deploy.timeout`** (optional): HTTP request timeout in seconds for the job runner when calling this backend (default: `10`). Increase this for long-running operations like LLM inference.
- **`deploy.timeout_overridable`** (optional): When `true`, the frontend can override the timeout per-job via the `timeout` option in `db.run()`. When `false` (default), the deploy-time timeout is always used.
...
<CodeBlock
  ...
  agent:
    ...
    deploy:
      ...
      timeout_overridable: true   # Let the frontend set per-job timeouts`}
  lang="yaml"
/>

When `timeout_overridable` is `true`, the frontend can pass a `timeout` option to `db.run()`:
...
If `timeout_overridable` is `false` (the default), the `timeout` option in `db.run()` is ignored and the deploy-time value is always used.
...
- **`timeout`**: Optional per-job timeout override in seconds (only used if backend has `timeout_overridable: true`)
...
- **`backend`**: The name of the backend from your `sp00ky.yml` (e.g., `'api'`)
...
- **`options`** (optional):
  ...
  - **`timeout`**: Override the backend HTTP request timeout in seconds. Only takes effect if the backend has `timeout_overridable: true` in `sp00ky.yml`.
...
Each backend in `sp00ky.yml` can declare an optional `dev` field. When you run `spky dev`, these commands are spawned alongside the rest of the infrastructure (SurrealDB, SSP, scheduler), so you don't need to start backend services in separate terminals.
...
<CodeBlock
  ...
  api:
    dev:
      type: npm
      script: dev
      env-file: .env.development   # loaded into the process environment
      workdir: ../api
    # ...other fields...`}
  lang="yaml"
/>
...
// Long-running AI operation (requires timeout_overridable: true on backend)
