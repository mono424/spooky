...
export const code20 = `mode: sidecar
apps:
  api:
    type: backend
...
export const code18 = `// Use a 60-second timeout for this specific call
await db.run('agent', '/chat', { message: 'Hello' }, {
  timeout: 60
});`;

export const code17 = `# Backend with authentication
apps:
  api:
    type: backend
...
export const code16 = `import { bearerAuth } from 'hono/bearer-auth';

app.use('/*', bearerAuth({
  token: process.env.API_AUTH_TOKEN
}));`;

export const code15 = `openapi: 3.1.0
info:
  version: 1.0.0
  title: example
paths:
...
export const code14 = `DEFINE TABLE job SCHEMAFULL
  PERMISSIONS
...
export const code13 = `// Call a backend function
await db.run('api', '/spookify', { id: threadData.id }, { 
  assignedTo: threadData.id 
});`;

export const code12 = `db.run(
...
export const code11 = `const threadQuery = db.query('thread')
...
export const code10 = `import { useQuery } from '@spooky-sync/client-solid';
import { db } from './db';
...
export const code4 = `await db.run('api', '/process-order', 
  { orderId: order.id },
  { assignedTo: order.id }  // Link job to order
);`;

export const code3 = `const job = thread()?.jobs?.[0];
...
export const code2 = `// Safe to retry multiple times
...
export const code1 = `DEFINE TABLE job SCHEMAFULL
  PERMISSIONS
    FOR select, create, update, delete 
    WHERE $access = "account" AND assigned_to.author.id = $auth.id;`;
