...
logs, bookkeeping, internal secrets, or anything that should live in your database but never reach a browser.

<CodeBlock code={nosyncCode} lang="sql" />

A `@nosync` table or field is:
...
<Warning title="Not the same as locking permissions">
  `@nosync` is distinct from `PERMISSIONS FOR select WHERE false`. A permission-locked table or field is
  still synced (it shows up in generated types and the snapshot, it just can't be read); `@nosync`
  removes the table or field from sync altogether. Reach for `@nosync` when data should never leave the
  server.
</Warning>
