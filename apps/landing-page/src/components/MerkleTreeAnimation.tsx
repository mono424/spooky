...
export const MerkleTree = () => {
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center p-4 bg-[#0a0a0a] border border-[#333] rounded-sm relative min-h-[400px]">
      ...
      <MerkleSVG activeNodes={activeNodes} hashingNode={hashingNode} nodeHashes={nodeHashes} />

      <div className="mt-4 text-[9px] text-gray-500 font-mono absolute bottom-4">
        STATUS:{' '}
        <span className={hashingNode !== null ? 'text-purple-400 animate-pulse' : 'text-gray-700'}>
      ...
      <div className="border border-white/10 bg-white/[0.02] pl-6 py-4 relative flex flex-col gap-8 min-w-0">
        ...
        <div className="flex flex-col justify-between">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div
                className={`text-sm font-mono tracking-wide transition-colors duration-300 break-all ${hashingNode === 0 ? 'text-purple-400' : 'text-gray-500'}`}
              >
                {nodeHashes[0] || '0x2ab6'}
              </div>
            </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 border-t border-white/5 pt-6">
          <pre className="text-[10px] md:text-xs text-gray-500 font-mono leading-relaxed whitespace-pre-wrap break-words">
            ...
            {'  '}
            <span className="text-blue-400/70">merkle_root</span>:{' '}
            <span className="text-orange-300/70">{nodeHashes[0] || '0x3e4d...'}</span>...{'\n'}
            &#125;;
          </pre>

          <div className="mt-4 pt-4 text-[10px] text-gray-600 leading-relaxed">
            <span className="text-gray-400 font-bold">Note:</span> Live queries subscribe to root
            hashes. Data syncs when client/server hashes match.
          </div>
        </div>
      </div>
    </div>
