...
const GROUPS = {
  TABLES: [
    {
      ...
      kind: 'table',
      type: 'SCHEMAFULL',
      doc: 'System users and authentication details.',
    },
    {
      label: 'thread',
      kind: 'table',
      type: 'SCHEMAFULL',
      doc: 'Root collection for discussion threads. Contains title, body, and author references.',
    },
  ...
  RELATIONS: [
    ...
    { label: 'comments', kind: 'relation', type: 'array<comment>', doc: 'Replies to this thread.' },
    {
      ...
      kind: 'relation',
      type: 'array<category>',
      doc: 'Tags associated with thread.',
    },
  ] as SuggestionItem[],
  ORDER_FIELDS: [
...
const getContextForValue = (val: string) => {
  if (GROUPS.TABLES.find((i) => i.label === val))
    return { options: GROUPS.TABLES, title: 'Table Schema', isReadOnly: true };
  if (GROUPS.RELATIONS.find((i) => i.label === val))
    return { options: GROUPS.RELATIONS, title: 'Select Relation', isReadOnly: false };
  if (GROUPS.ORDER_FIELDS.find((i) => i.label === val))
    return { options: GROUPS.ORDER_FIELDS, title: 'Order By', isReadOnly: false };
  if (GROUPS.DIRECTIONS.find((i) => i.label === val))
    return { options: GROUPS.DIRECTIONS, title: 'Direction', isReadOnly: false };
  return null;
};
  ...
  table: (
    <svg className="w-3 h-3 text-orange-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path
        ...
        strokeLinejoin="round"
        strokeWidth={2}
        d="M3 10h18M3 14h18m-9-4v8m-7-4h14M2 5h20v14H2V5z"
      />
    </svg>
  ),
  relation: (
        ...
        {isOpen && (
          <div className="absolute left-0 top-full mt-1 w-[280px] bg-[#1a1a1a] border border-[#333] shadow-[0_8px_32px_rgba(0,0,0,0.9)] rounded-sm font-mono text-xs animate-in fade-in slide-in-from-top-1 duration-150 pointer-events-auto cursor-default z-30">
            ...
            <div className="bg-[#111] px-3 py-1.5 border-t border-[#222] text-[10px] text-gray-600">
              <span className="text-blue-500">★</span> Primary Key:{' '}
  ...
  const renderCode = (text: string) => {
    ...
    return tokens.map((token, index) => {
      ...
      const parts = token.split(
        /(thread\.author\.username|\b(?:useQuery|db|query|related|orderBy|limit|build|import|export|from|const|return|function|default|type)\b|[{}[\]().,;]|\s+)/g
      );
    ...
    <div className="border border-surface-border bg-surface shadow-xl flex flex-col font-mono text-sm relative overflow-visible group rounded-xl min-h-[320px]">
      ...
      >
        <div className="flex gap-4">
          <span className="flex items-center gap-2 text-gray-500">SP00KY_SYNC</span>
        </div>
